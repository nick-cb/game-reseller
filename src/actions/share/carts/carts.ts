...
export async function addItemToCart(params: AddToCartParams) {
  try {
    ...
    const { data, error: gameErr } = await ShareActions.games.getMinimalInfo({ ID: game.ID });
    if (gameErr) {
      throw new Error(gameErr.message);
    }
    const { data: user, error: userErr } = await ShareActions.users.getUserByID(payload.user.ID);
    if (userErr) {
