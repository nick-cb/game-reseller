...
import { VideoContainer } from '@/components/media/Video2';
import { AudioContainer } from '@/components/media/Audio2';
import { getAudioSourcesFromVideo, getVideoSources } from '@/utils';
import {
  IntersectionObserverContainer,
  IntersectionObserverRoot,
} from '../../intersection/IntersectionObserver';
import { ScrollContainer, ScrollItem, VideoScrollItem } from '../../scroll2/ScrollPrimitive';
...
export default function GameItemCarousel(props: LinearCarouselProps) {
  ...
  return (
    <div>
      <IntersectionObserverContainer>
        <IntersectionObserverRoot>
          <ul className="group relative overflow-hidden">
            {/* <NextPrevControls totalItems={videos.length + images.length} /> */}
            <ScrollContainer className="scrollbar-hidden flex snap-x snap-mandatory overflow-scroll">
              {videos.map((video) => {
                return (
